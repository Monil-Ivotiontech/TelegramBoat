...
export class TestingComponent implements OnInit {
  ...
  client_id_tables_list: Array<any> = [];
  client_id_switched_table_list_name: string = "manager";
  ...
  bot_access_switched_table_list_name: string = "venus_manager";
  ...
  group_names_switched_table_list_name: string = "venus_manager";
  ...
  onTabChange(event: { index: number; }) {
    if (event.index == 0) {
      ...
      this.client_id_tables_list = [{ label: 'Manager', value: 'manager' }, { label: 'User', value: 'user' }];
  ...
  confirm_manager_chat() {
    ...
    if (this.available_chatid != "" || this.available_chatid == 0) {
      ...
      const credentials = { "userid": userid, "email": email, "managerloginid": managerloginid, "chatid": chatid, "commandaccess": commandaccess };
  ...
  client_id_table_name_switched(event: any) {
    this.client_id_switched_table_list_name = event.option.value;
  ...
  confirm_venus_manager_mapped_data_delete() {
    ...
    this.httpService.frontendRequestCall(EndPoints.delete_user_from_venus_manager_mapping, ApiMethod.POST, credentials)
      .subscribe(response => {
        ...
        if (status == "true") {
          this.ProgressSpinnerDlg = false;
          this.get_venus_manager_mapping_detail(this.selected_view_group_mapping_details);
          this.messageService.add({ severity: 'success', summary: 'Success', detail: message });
  ...
  confirm_venus_cmd_updation() {
    ...
    let commissiontype = this.update_venus_cmd_form.controls.commission_type.value;
    const credentials = { "userid": userid, "email": email, "commandid": commandid, "commissiontype": commissiontype };
